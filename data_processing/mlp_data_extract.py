...
class DataHandling:
    def __init__(self, data=r"data/train_data.csv"):
        ...
        # For logging predictions for GNN training data 
        self.predictions = []
        self.probabilities = []
...
def pipeline(handler):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(handler.X, handler.y, handler.groups)):
        ...
        results_df.to_csv('data/less_dense/ld_surrogate_data.csv', index=False)
