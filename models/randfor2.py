...
def run_rf_cross_validation(df):
    ...
    # Initialize list to store all test predictions from all folds
    ...
    for fold, (train_idx, test_idx) in enumerate(group_kfold.split(X, y, groups)):
        ...
        fold_results_df = X_test.copy()
        fold_results_df['true_label'] = y_test.values
        fold_results_df['predicted_label'] = y_pred
        fold_results_df['prob_class_1'] = probs[:, 1]  # Probability for class 1
        
        # Add metadata
        fold_results_df['patient_id'] = df.iloc[test_idx]['patient_id'].values
        fold_results_df['chunk'] = df.iloc[test_idx]['chunk'].values
        fold_results_df['fold'] = fold + 1
        fold_results_df['test_idx'] = test_idx  # Original index in dataset
        
        # Add to master list
        all_test_predictions.append(fold_results_df)
    ...
    # Combine all test predictions from all folds
    combined_predictions_df = pd.concat(all_test_predictions, ignore_index=True)
    
    # Save to CSV
    output_file = 'data/rf_all_test_predictions.csv'
    combined_predictions_df.to_csv(output_file, index=False)
    print(f"\n=== SAVED ALL TEST PREDICTIONS ===")
    print(f"Total test predictions saved: {len(combined_predictions_df)}")
    print(f"File: {output_file}")
    print(f"Columns: {list(combined_predictions_df.columns)}")
    
    # Print summary of predictions
    print(f"\nPrediction Summary:")
    print(f"Samples per fold:")
    print(combined_predictions_df['fold'].value_counts().sort_index())
    print(f"\nOverall accuracy across all folds: {(combined_predictions_df['true_label'] == combined_predictions_df['predicted_label']).mean():.4f}")
    
    return all_reports, all_conf_matrices, fold_details, all_roc_aucs, rf, combined_predictions_df
...
if __name__ == "__main__":
    ...
    if results[0] is not None:
        all_reports, all_conf_matrices, fold_details, all_roc_aucs, rf, combined_predictions = results
        print("\nStep 3: Summarize Results")
        summarize_rf_results(all_reports, all_conf_matrices, fold_details, all_roc_aucs)
        
        print(f"\n=== FINAL DATASET FOR SURROGATE MODELING ===")
        print(f"Combined predictions shape: {combined_predictions.shape}")
        print(f"Available for surrogate training: {len(combined_predictions)} samples")
        print(f"Features available: {len([col for col in combined_predictions.columns if col not in ['true_label', 'predicted_label', 'prob_class_1', 'patient_id', 'chunk', 'fold', 'test_idx']])}")
