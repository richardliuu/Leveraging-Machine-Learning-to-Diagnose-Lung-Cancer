...
# Suppress specific scipy warnings about WAV chunks not understood
warnings.filterwarnings("ignore", category=wavfile.WavFileWarning)

def process_dataset(csv_path, wav_folder_healthy, wav_folder_unhealthy=None, output_path=None):
    """
    ...
    # Process unhealthy files if provided
    if wav_folder_unhealthy:
        stats_unhealthy = process_wav_files(df, wav_folder_unhealthy, 'u')
        stats['processed_files'] += stats_unhealthy['processed']
        stats['missing_files'].extend(stats_unhealthy['missing'])
        stats['error_files'].extend(stats_unhealthy['errors'])
    ...
    for subject_id in df.index:
        ...
        if os.path.exists(file_path):
            try:
                ...
                # Create column name based on file type
                column_name = 'length_healthy' if file_suffix == 'h' else 'length_unhealthy'
                if column_name not in df.columns:
                    df[column_name] = np.nan
                
                # Update DataFrame
                df.at[subject_id, column_name] = duration
...
if __name__ == "__main__":
    # Configuration
    csv_path = "C:/Users/richa/Downloads/lung_dataset_unhealthy.csv"
    wav_folder_healthy = "wavfiles/healthy"
    wav_folder_unhealthy = "wavfiles/unhealthy"  # Set to None if you don't have unhealthy files
    output_path = "C:/Users/richa/Downloads/lung_dataset_with_lengths.csv"
    
    # Process the dataset
    updated_df, stats = process_dataset(
        csv_path=csv_path,
        wav_folder_healthy=wav_folder_healthy,
        wav_folder_unhealthy=None,  # Change to wav_folder_unhealthy if you have unhealthy files
